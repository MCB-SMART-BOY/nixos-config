...
克隆后 `cp local.nix.example local.nix`，编辑填入你的用户名/主机名/时区即可部署。
硬件配置由 `nixos-generate-config` 生成到 `hardware-configuration.nix`（gitignored）。
...
├── local.nix.example           # 机器身份模板（新用户从这里开始）
...
5. **选项集中管理** — 所有 `mcb.*` 选项声明在 `modules/options.nix`，模块只读取 `config.mcb.*`
6. **预置用户只有一个** — `admin`，其他通过 `./run.sh add-user` 生成
