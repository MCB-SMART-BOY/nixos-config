...
fn usage() {
    eprintln!(
        "Usage:\n  update-upstream-apps-rs          # update source pins\n  update-upstream-apps-rs --check  # check source pins are up-to-date"
    );
}
...
fn main() {
    ...
    let result = run("update-zed-source-rs", check)
        .and_then(|_| run("update-yesplaymusic-source-rs", check));
    match result {
        Ok(_) => {
            if check {
        ...
        Err(err) => {
            eprintln!("update-upstream-apps-rs: {err:#}");
            std::process::exit(1);
        }
    }
}
