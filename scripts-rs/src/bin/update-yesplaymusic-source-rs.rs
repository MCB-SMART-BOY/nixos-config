...
fn usage() {
    eprintln!(
        "Usage:\n  update-yesplaymusic-source-rs          # update source.nix to latest upstream stable release\n  update-yesplaymusic-source-rs --check  # check whether source.nix is up-to-date"
    );
}
...
fn main_impl() -> Result<i32> {
    ...
    if check_only {
        ...
        if command_exists("diff") && source_file.is_file() {
            let tmp = PathBuf::from("/tmp/yesplaymusic-source-rs.nix");
...
fn main() {
    match main_impl() {
        ...
        Err(err) => {
            eprintln!("update-yesplaymusic-source-rs: {err:#}");
            std::process::exit(1);
        }
    }
}
