...
let
  ...
  # GPU wrapper 脚本（bash 源码在 users/admin/scripts/）
  gpuModeCurrent = pkgs.writeShellApplication {
    ...
    text = builtins.readFile ./scripts/gpu-current.sh;
  };

  zedAutoGpu = pkgs.writeShellApplication {
    name = "zed-auto-gpu";
    runtimeInputs = [ gpuModeCurrent ];
    text = builtins.readFile ./scripts/zed-gpu.sh;
  };

  electronAutoGpu = pkgs.writeShellApplication {
    name = "electron-auto-gpu";
    runtimeInputs = [ gpuModeCurrent ];
    text = builtins.readFile ./scripts/electron-gpu.sh;
  };
in
{
  options.mcb = {
  ...
  config = {
    ...
    programs.noctalia-shell.settings =
      if noctaliaCfg.barProfile == "default" then defaultNoctaliaSettings else { };
