...
- `mcbctl/src/tui/state/packages/`
  `Packages` 页进一步拆成 `browse.rs`、`support.rs`、`confirm.rs` 和 `mutate_*` 分片，不再把浏览、搜索、分组编辑、确认逻辑和落盘全堆在一个文件里
...
- `mcbctl/src/bin/control/mcb-deploy/source/`
  来源准备分拆层；`prompt.rs`、`local.rs`、`remote.rs` 分别负责来源选择、本地源准备和远端拉取/镜像重试
...
- `mcbctl/src/bin/noctalia/noctalia-gpu-mode/state/`
  GPU 拓扑探测、当前模式探测、模式列表发现与状态输出分拆层
