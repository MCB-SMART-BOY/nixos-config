...
fn main() {
    ...
    if !wait_for_wayland() {
        eprintln!("wallpaper-random: Wayland socket not ready");
        std::process::exit(1);
    }
    ...
    if wallpapers.is_empty() {
        eprintln!("wallpaper-random: no wallpapers found");
        std::process::exit(1);
    }
    ...
    let Some(choice) = wallpapers.choose(&mut rng).cloned() else {
        eprintln!("wallpaper-random: no wallpaper selected");
        std::process::exit(1);
    };
    ...
    if !command_exists("noctalia-shell") {
        eprintln!("wallpaper-random: noctalia-shell not found in PATH");
        std::process::exit(1);
    }
