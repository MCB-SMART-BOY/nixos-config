...
说明:
  ...
  release 模式用于发布新版本：更新 VERSION、创建 tag，并发布 GitHub Release。"
    ...
    fn host_exists(&self, repo_dir: &Path) -> bool {
        !self.target_name.is_empty() && repo_dir.join("hosts").join(&self.target_name).is_dir()
    }

    fn resolve_host_template(&self, repo_dir: &Path) -> Option<(String, PathBuf)> {
    ...
    fn resolve_user_template(&self, repo_dir: &Path) -> Option<(String, PathBuf)> {
    ...
    fn build_deploy_plan(&self) -> DeployPlan {
        let source = self.deploy_plan_source();
        let mut notes = Vec::new();

        if source == DeploySource::CurrentRepo {
            if let Some(local_repo) = self.detect_local_repo_dir() {
    ...
    fn rebuild_plan(&self) -> NixosRebuildPlan {
        NixosRebuildPlan {
            action: self.deploy_plan_action(),
    ...
    fn repo_sync_plan(&self, repo_dir: &Path) -> RepoSyncPlan {
        RepoSyncPlan {
    ...
    fn deploy_plan_task(&self) -> DeployTask {
    ...
    fn deploy_plan_source(&self) -> DeploySource {
        if self.force_remote_source {
            if self.allow_remote_head {
    ...
    fn deploy_plan_source_detail(&self) -> Option<String> {
        let source = self.deploy_plan_source();
        match source {
            DeploySource::RemotePinned => {
                if self.source_ref.is_empty() {
    ...
    fn deploy_plan_action(&self) -> DeployAction {
...
"#,
    ...
    fn ensure_host_entry(&mut self, repo_dir: &Path) -> Result<()> {
        if self.target_name.is_empty() {
            bail!("未指定主机名称。");
        }
