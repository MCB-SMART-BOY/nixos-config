...
fn usage() {
    eprintln!(
        "Usage:\n  update-zed-source          # update source.nix to latest upstream stable release\n  update-zed-source --check  # check whether source.nix is up-to-date"
    );
}
...
fn main_impl() -> Result<i32> {
    ...
    if check_only {
        ...
        if command_exists("diff") && source_file.is_file() {
            let tmp = PathBuf::from("/tmp/zed-source.nix");
...
fn main() {
    match main_impl() {
        ...
        Err(err) => {
            eprintln!("update-zed-source: {err:#}");
            std::process::exit(1);
        }
    }
}
